...
#define LOG_TAG "vendor.evervolv.touch@1.0-service.oneplus_holi"
...
constexpr const char* kGestureSwitchPath = "/proc/touchpanel/gesture_enable";
...
Return<bool> TouchscreenGesture::setGestureEnabled(const Gesture& gesture, bool enabled) {
    ...
    if (ReadFileToString(kGestureSwitchPath, &tmp)) {
    ...
    return WriteStringToFile(std::to_string(contents), kGestureSwitchPath, true);
}
